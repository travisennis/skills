...
function listDaemonSockets() {
  return readdirSync('/tmp')
    ...
    .map(f => ({ targetId: f.slice(4, -5), socketPath: `/tmp/${f}` }));
}
...
async function shotStr(cdp, sid, filePath) {
  ...
  if (dpr !== 1) {
    lines.push(`  On this ${dpr}x display: CSS px = screenshot px / ${dpr} (screenshot px = CSS px * ${Math.round(100*dpr)/100})`);
  }
  return lines.join('\n');
}
...
Usage: cdp <command> [args]
  ...
  evalraw <target> <method> [json]
                            Send a raw CDP command; returns JSON result
...
COORDINATE SYSTEM
shot captures the viewport at the device's native resolution. The screenshot
image size = CSS pixels * DPR (device pixel ratio). For CDP Input events
(clickxy, etc.) you need CSS pixels, not image pixels.
...
Each tab runs a persistent daemon at Unix socket:
